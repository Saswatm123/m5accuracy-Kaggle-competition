...
```{r}
undiff = function(x, order, base){
  #x is the diff vector
  ...
  forecasted.zero.diff = c(0)
  while(length(diff.levels[[order]]) != 0){
  ...
  forecasted.zero.diff
}

diff.levels = undiff(price.dd, order = 2, base = vol.by.date$price[1:2])
```
